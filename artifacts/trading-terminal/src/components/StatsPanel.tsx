...
export function StatsPanel({ symbol }: { symbol: string }) {
  ...
  if (!stats) return <div className="p-4 text-muted-foreground font-mono text-sm">Loading stats...</div>;
    ...
    <div className="flex h-full p-4 gap-6">
      <div className="w-[300px] flex flex-col justify-center gap-4 border-r border-border/50 pr-6">
        <div className="flex justify-between items-end">
          ...
          <div className={`text-3xl font-bold font-mono ${stats.winRate >= 50 ? 'text-green-500' : 'text-red-500'}`}>
            {stats.winRate.toFixed(1)}%
          </div>
        </div>
        <div className="h-2 w-full bg-red-500/20 rounded-full overflow-hidden flex">
          <div className="h-full bg-green-500 transition-all duration-500" style={{ width: `${stats.winRate}%` }} />
        </div>
            ...
            <tbody>
              {history?.filter(t => t.outcome !== null).map(trade => (
                <tr key={trade.id} className="border-t border-border/20">
                  ...
                  <td className="py-2">{trade.entryPrice.toFixed(5)}</td>
                  <td className="py-2">{trade.exitPrice?.toFixed(5) || '---'}</td>
                  <td className="py-2">
